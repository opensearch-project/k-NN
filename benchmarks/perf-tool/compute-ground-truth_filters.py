...
def generate_ground_truth(in_file_path, out_file_path):
    ...
    validationDataSet = HDF5DataSet(in_file_path, "neighbors")
    actualNeighbors = validationDataSet.read_neighbors(0, 10000)

    validationDataSet_dis = HDF5DataSet(in_file_path, "distances")
    actualNeighbors_dis = validationDataSet_dis.read_neighbors(0, 10000)
    ...
    for i in range(len(actualNeighbors[0])):
        if actualNeighbors[0][i] != results[0][i] or actualNeighbors_dis[0][i] != ids_distance[0][i] :
            print(f'Failed at {i}, a : {actualNeighbors[0][i]} r: {results[0][i]}')
...
def main2(argv):
    opts, args = getopt.getopt(argv, "", ["input_file=", "output_file="])
    ...
    generate_ground_truth(in_file_path=inputfile, out_file_path=outputfile)
