...
public class Lucene102RWBinaryQuantizedVectorsFormat extends Lucene102BinaryQuantizedVectorsFormat {
    ...
    private static final Lucene102RWBinaryFlatVectorsScorer rwScorer = new Lucene102RWBinaryFlatVectorsScorer(
        FlatVectorScorerUtil.getLucene99FlatVectorsScorer()
    );

    @Override
    public FlatVectorsWriter fieldsWriter(SegmentWriteState state) throws IOException {
        return new Lucene102BinaryQuantizedVectorsWriter(rwScorer, rawVectorFormat.fieldsWriter(state), state);
    }
}
