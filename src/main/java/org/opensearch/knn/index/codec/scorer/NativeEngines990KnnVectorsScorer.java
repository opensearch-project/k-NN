...
public class NativeEngines990KnnVectorsScorer implements FlatVectorsScorer {
    ...
    /**
     * Resolves the native similarity function type for an fp16 (half_float) index. Half_float storage is
     * never bf16, so this delegates to the bit-width-aware overload with {@code isBF16 = false}.
     */
    public static SimdVectorComputeService.SimilarityFunctionType getNativeFunctionType(final VectorSimilarityFunction similarityFunction) {
        return getNativeFunctionType(similarityFunction, false);
    }
    ...
    public static SimdVectorComputeService.SimilarityFunctionType getNativeFunctionType(
        ...
        return switch (similarityFunction) {
            ...
            // BF16 has no native cosine kernel, so BF16 + COSINE falls back to the delegate scorer.
            case COSINE -> isBF16 ? null : SimdVectorComputeService.SimilarityFunctionType.FP16_COSINE;
            default -> null;
        };
    }
}
