 ...
 * #writeSortingField} remaps to new-doc order first, and {@link #mergeOneField} writes
...
public class KNN1040HalfFloatFlatVectorsWriter extends FlatVectorsWriter {
    ...
    /**
     * Merges the field like {@link #mergeOneField}, but additionally hands back a scorer supplier over
     * the just-merged vectors so a wrapping HNSW writer can rebuild its graph without re-reading the
     * source segments. Mirrors {@code Lucene99FlatVectorsWriter.mergeOneFieldToIndex}: the merged FP16
     * bytes go to a temp file first (an {@link IndexOutput} can't be read back), that file is copied
     * into {@code .vec} at the field's aligned offset, and the temp file is then reopened read-only to
     * back the supplier. The returned supplier owns the temp input and deletes the temp file on close.
     */
    @Override
    public CloseableRandomVectorScorerSupplier mergeOneFieldToIndex(FieldInfo fieldInfo, MergeState mergeState) throws IOException {
        ...
        IndexInput tempVectorInput = null;
        boolean success = false;
        try {
            final DocsWithFieldSet docsWithField = writeVectorData(tempVectorData, mergedValues, dimension);
            CodecUtil.writeFooter(tempVectorData);
            IOUtils.close(tempVectorData);

            tempVectorInput = segmentWriteState.directory.openInput(tempVectorData.getName(), segmentWriteState.context);
            final long vectorDataOffset = alignOutput(vectorData);
            vectorData.copyBytes(tempVectorInput, tempVectorInput.length() - CodecUtil.footerLength());
            CodecUtil.retrieveChecksum(tempVectorInput);
            final long vectorDataLength = vectorData.getFilePointer() - vectorDataOffset;

            writeMeta(fieldInfo, segmentWriteState.segmentInfo.maxDoc(), vectorDataOffset, vectorDataLength, docsWithField);

            final int count = docsWithField.cardinality();
            // Ordinals are dense in the merged segment, so no ord-to-doc mapping is needed here: the
            // graph builder addresses vectors by ordinal only.
            final KNN1040HalfFloatFlatVectorsValues vectorValues = new KNN1040HalfFloatFlatVectorsValues(
                dimension,
                count,
                tempVectorInput.slice("merged FP16 vectors", 0, (long) count * dimension * Short.BYTES),
                null,
                fieldInfo.getVectorSimilarityFunction()
            );
            final RandomVectorScorerSupplier supplier = vectorsScorer.getRandomVectorScorerSupplier(
                fieldInfo.getVectorSimilarityFunction(),
                vectorValues
            );
            final IndexInput finalTempVectorInput = tempVectorInput;
            ...
            return new CloseableRandomVectorScorerSupplier() {
                ...
                public void close() throws IOException {
                    IOUtils.close(finalTempVectorInput);
                    IOUtils.deleteFilesIgnoringExceptions(segmentWriteState.directory, tempFileName);
                }
            };
        } finally {
            if (!success) {
                IOUtils.closeWhileHandlingException(tempVectorData, tempVectorInput);
