 ...
 */
class KNN91DocValuesConsumer extends DocValuesConsumer implements Closeable {

    private final Logger logger = LogManager.getLogger(KNN91DocValuesConsumer.class);

    private final String TEMP_SUFFIX = "tmp";
    private DocValuesConsumer delegatee;
    private SegmentWriteState state;

    KNN91DocValuesConsumer(DocValuesConsumer delegatee, SegmentWriteState state) throws IOException {
        ...
        try {
            ...
            for (FieldInfo fieldInfo : mergeState.mergeFieldInfos) {
                ...
                if (type == DocValuesType.BINARY && fieldInfo.attributes().containsKey(KNNVectorFieldMapper.KNN_FIELD)) {
                    addKNNBinaryField(fieldInfo, new KNN91DocValuesReader(mergeState));
                }
            }
        } catch (Exception e) {
