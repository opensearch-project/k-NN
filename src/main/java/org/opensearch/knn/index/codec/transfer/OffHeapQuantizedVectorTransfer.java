...
import static org.opensearch.knn.common.KNNVectorUtil.createArrayList;

/**
 * The class is intended to transfer {@link KNNVectorValues} to off heap memory. If also provides and ability to quantize the vector
 * before it is transferred to offHeap memory. The class is not thread safe
 *
 * @param <T> an array of primitive type
 * @param <V> an array of primitive type after being quantized
 */
abstract class OffHeapQuantizedVectorTransfer<T, V> implements VectorTransfer {
    ...
    public OffHeapQuantizedVectorTransfer(
        ...
        this.vectorsToTransfer = createArrayList(this.batchSize);
        this.transferredDocIdsList = createArrayList(this.batchSize);
    ...
    public void transferBatch() throws IOException {
        ...
        // Handle batchSize < transferLimit
