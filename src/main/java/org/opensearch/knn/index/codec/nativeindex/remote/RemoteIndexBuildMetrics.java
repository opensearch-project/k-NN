    ...
    public void endRemoteIndexBuildMetrics(BuildResult buildResult) {
        ...
        switch (buildResult) {
            ...
            case MERGE_ABORT:
                ...
                log.debug("Remote index build aborted after {} ms for vector field [{}]", time_in_millis, fieldName);
                break;
            case TERMINAL_IO:
                INDEX_BUILD_TERMINAL_EXCEPTION.increment();
                log.debug("Remote index build terminated after {} ms for vector field [{}]", time_in_millis, fieldName);
                break;
            case FAILURE:
            default:
