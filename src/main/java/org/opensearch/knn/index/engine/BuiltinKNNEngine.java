 ...
 */
public enum BuiltinKNNEngine implements KNNLibrary, KNNEngine {
    ...
    private static final Set<KNNEngine> CUSTOM_SEGMENT_FILE_ENGINES = ImmutableSet.of(BuiltinKNNEngine.NMSLIB, BuiltinKNNEngine.FAISS);
    private static final Set<KNNEngine> ENGINES_SUPPORTING_FILTERS = ImmutableSet.of(BuiltinKNNEngine.LUCENE, BuiltinKNNEngine.FAISS);
    public static final Set<KNNEngine> ENGINES_SUPPORTING_RADIAL_SEARCH = ImmutableSet.of(BuiltinKNNEngine.LUCENE, BuiltinKNNEngine.FAISS);
    public static final Set<KNNEngine> DEPRECATED_ENGINES = ImmutableSet.of(BuiltinKNNEngine.NMSLIB);
    public static final Set<KNNEngine> ENGINES_SUPPORTING_NESTED_FIELDS = ImmutableSet.of(BuiltinKNNEngine.LUCENE, BuiltinKNNEngine.FAISS);
     ...
     */
    BuiltinKNNEngine(String name, KNNLibrary knnLibrary) {
        this.name = name;
     ...
     */
    BuiltinKNNEngine(String name, KNNLibrary knnLibrary, Version restrictedVersion) {
        this.name = name;
        this.knnLibrary = knnLibrary;
    ...
    public static KNNEngine getEngineNameFromPath(String path) {
        if (path.endsWith(BuiltinKNNEngine.NMSLIB.getExtension()) || path.endsWith(BuiltinKNNEngine.NMSLIB.getCompoundExtension())) {
            return BuiltinKNNEngine.NMSLIB;
        }

        if (path.endsWith(BuiltinKNNEngine.FAISS.getExtension()) || path.endsWith(BuiltinKNNEngine.FAISS.getCompoundExtension())) {
            return BuiltinKNNEngine.FAISS;
        }
    ...
    public static int getMaxDimensionByEngine(KNNEngine knnEngine) {
        return MAX_DIMENSIONS_BY_ENGINE.getOrDefault(knnEngine, MAX_DIMENSIONS_BY_ENGINE.get(BuiltinKNNEngine.DEFAULT));
    }
