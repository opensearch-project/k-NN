...
public abstract class AbstractLuceneMethod extends AbstractKNNMethod {
    ...
    /**
     * Normalizes half_float cosine vectors on write. Upstream does this because its FP16_COSINE kernel
     * computes {@code (1 + dot) / 2}, which equals cosine only for unit vectors; this branch has no such
     * kernel and scores cosine directly, where normalization is scale-neutral, so the behavior is kept
     * identical to upstream. float32 is excluded - Lucene's quantized writer handles it.
     */
    @Override
