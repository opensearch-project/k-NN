...
public class MuveraSearchRequestProcessor extends AbstractProcessor implements SearchRequestProcessor {
    ...
    public SearchRequest processRequest(SearchRequest request) throws Exception {
        ...
        ScriptScoreQueryBuilder scriptScoreQuery = (ScriptScoreQueryBuilder) query;
        ...
        // Extract multi-vectors from script params
        ...
        int numTokens = queryVectorsList.size();
        if (numTokens == 0) {
            throw new IllegalArgumentException("[" + QUERY_VECTORS_PARAM + "] must not be empty");
        }

        // Convert to double[][] for MUVERA encoding, validating dimensions
        double[][] multiVectors = new double[numTokens][dim];
        for (int t = 0; t < numTokens; t++) {
            List<Number> tokenVec = queryVectorsList.get(t);
            ...
            for (int d = 0; d < dim; d++) {
                multiVectors[t][d] = tokenVec.get(d).doubleValue();
            }
        }
        ...
        // Rebuild script_score with knn as inner query, keeping the original script untouched
