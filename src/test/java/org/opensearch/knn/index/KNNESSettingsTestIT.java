...
public class KNNESSettingsTestIT extends KNNRestTestCase {
    ...
    public void testIndexWritesPluginDisabled() throws Exception {
        ...
        // disable plugin
        updateClusterSettings(KNN_PLUGIN_ENABLED, false);
        ...
        // enable plugin
        updateClusterSettings(KNN_PLUGIN_ENABLED, true);
    ...
    public void testItemRemovedFromCache_expiration() throws Exception {
        createKnnIndex(INDEX_NAME, createKnnIndexMapping(FIELD_NAME, 2));
        updateClusterSettings(KNN_CACHE_ITEM_EXPIRY_ENABLED, true);
        ...
        updateClusterSettings(KNN_CACHE_ITEM_EXPIRY_ENABLED, false);
    }
    ...
    public void testUpdateIndexSetting() throws IOException {
        Settings settings = Settings.builder().put("index.knn", true).put(KNN_ALGO_PARAM_EF_SEARCH, 512).build();
        createKnnIndex(INDEX_NAME, settings, createKnnIndexMapping(FIELD_NAME, 2));
        assertEquals("512", getIndexSettingByName(INDEX_NAME, KNN_ALGO_PARAM_EF_SEARCH));

        updateIndexSettings(INDEX_NAME, Settings.builder().put(KNN_ALGO_PARAM_EF_SEARCH, 400));
        assertEquals("400", getIndexSettingByName(INDEX_NAME, KNN_ALGO_PARAM_EF_SEARCH));

        Exception ex = expectThrows(
            ResponseException.class,
            () -> updateIndexSettings(INDEX_NAME, Settings.builder().put(KNN_ALGO_PARAM_EF_SEARCH, 1))
        );
