...
public class BasePerFieldKnnVectorsFormatTests extends KNNTestCase {
    ...
    private static class StubKnnVectorsFormat extends KnnVectorsFormat {
        StubKnnVectorsFormat() { super("stub"); }
        @Override public KnnVectorsWriter fieldsWriter(SegmentWriteState s) { return null; }
        @Override public KnnVectorsReader fieldsReader(SegmentReadState s) { return null; }
        @Override
        public int getMaxDimensions(String fieldName) { return 16000; }
    }
    ...
    private static KNNMethodContext getKnnMethodContext(KNNEngine engine) {
        return new KNNMethodContext(
            engine, SpaceType.L2, new MethodComponentContext(METHOD_HNSW, Collections.emptyMap())
        );
    }

    private static KNNMappingConfig getMappingConfig(
        CompressionLevel compressionLevel, KNNMethodContext knnMethodContext, Mode mode,
        Version indexCreatedVersion, Optional<String> topLevelEngine) {
        return new KNNMappingConfig() {
            @Override public Optional<KNNMethodContext> getKnnMethodContext() { return Optional.of(knnMethodContext); }
            @Override public int getDimension() { return 4; }
            @Override public Mode getMode() { return mode; }
            @Override public Version getIndexCreatedVersion() { return indexCreatedVersion; }
            @Override public CompressionLevel getCompressionLevel() { return compressionLevel; }
            @Override public Optional<String> getTopLevelKnnEngine() { return topLevelEngine; }
        };
    }
    ...
    public void testBasePerFieldKnnVectorsFormat_whenVersion36LuceneOnDisk_thenReturnBBQFormat() {
        ...
        KNNMappingConfig mappingConfig = getMappingConfig(
            CompressionLevel.NOT_CONFIGURED, knnMethodContext, Mode.ON_DISK, Version.V_3_6_0, Optional.empty());
    ...
    public void testBasePerFieldKnnVectorsFormat_whenVersion36LuceneCompression32x_thenReturnBBQFormat() {
        KNNMethodContext knnMethodContext = getKnnMethodContext(KNNEngine.LUCENE);
        KNNMappingConfig mappingConfig = getMappingConfig(
            CompressionLevel.x32, knnMethodContext, Mode.IN_MEMORY, Version.V_3_6_0, Optional.empty());
        Mockito.when(mockFieldType.getKnnMappingConfig()).thenReturn(mappingConfig);
    ...
    public void testBasePerFieldKnnVectorsFormat_whenVersion36LuceneTopLevelEngineCompression32x_thenReturnBBQFormat() {
        KNNMethodContext knnMethodContext = getKnnMethodContext(KNNEngine.UNDEFINED);
        KNNMappingConfig mappingConfig = getMappingConfig(
            CompressionLevel.x32, knnMethodContext, Mode.IN_MEMORY, Version.V_3_6_0, Optional.of(KNNEngine.LUCENE.getName()));
        Mockito.when(mockFieldType.getKnnMappingConfig()).thenReturn(mappingConfig);
    ...
    public void testBasePerFieldKnnVectorsFormat_whenVersionNot36LuceneOnDisk_thenDontReturnBBQFormat() {
        KNNMethodContext knnMethodContext = getKnnMethodContext(KNNEngine.LUCENE);
        KNNMappingConfig mappingConfig = getMappingConfig(
            CompressionLevel.NOT_CONFIGURED, knnMethodContext, Mode.ON_DISK, Version.V_3_3_0, Optional.empty());
        Mockito.when(mockFieldType.getKnnMappingConfig()).thenReturn(mappingConfig);
    ...
    public void testBasePerFieldKnnVectorsFormat_whenVersion36LuceneOnDiskNot32xCompression_thenDontReturnBBQFormat() {
        KNNMethodContext knnMethodContext = getKnnMethodContext(KNNEngine.LUCENE);
        KNNMappingConfig mappingConfig = getMappingConfig(
            CompressionLevel.x16, knnMethodContext, Mode.ON_DISK, Version.V_3_6_0, Optional.empty());
        Mockito.when(mockFieldType.getKnnMappingConfig()).thenReturn(mappingConfig);
