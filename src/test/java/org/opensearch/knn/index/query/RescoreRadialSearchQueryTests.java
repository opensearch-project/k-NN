...
public class RescoreRadialSearchQueryTests extends KNNTestCase {
    ...
    // Loose enough that every indexed vector clears the full-precision radius filter.
    private static final float LOOSE_RADIUS = 100.0f;
    ...
    // Given: a weight that is created but never scored
    // When: only explain/isCacheable are called on it
    // Then: the shard-wide first pass has not run — bounding candidates to the shard needs every
    // leaf at once, so it must not be paid by a weight Lucene never scores
    @SneakyThrows
    public void testCreateWeight_whenNeverScored_thenFirstPassIsDeferredAndMemoized() {
        final AtomicInteger firstPassRuns = new AtomicInteger();
        ...
        try (Directory directory = newDirectory()) {
            ...
            try (IndexReader reader = DirectoryReader.open(directory)) {
                ...
                RescoreRadialSearchQuery query = new RescoreRadialSearchQuery(
                    new CountingQuery(new MatchAllDocsQuery(), firstPassRuns),
                    FIELD_NAME,
                    QUERY_TARGET,
                    LOOSE_RADIUS,
                    false,
                ...
                Weight weight = query.createWeight(searcher, ScoreMode.COMPLETE, 1.0f);
                assertEquals("creating the weight must not run the first pass", 0, firstPassRuns.get());

                weight.isCacheable(leafContext);
                weight.explain(leafContext, 0);
                assertEquals("explain and isCacheable must not run the first pass", 0, firstPassRuns.get());

                assertNotNull(weight.scorerSupplier(leafContext));
                assertEquals("scoring runs the first pass", 1, firstPassRuns.get());

                weight.scorerSupplier(leafContext);
                assertEquals("the first pass is memoized", 1, firstPassRuns.get());
            }
        }
    }

    /** Wraps a query to count how many times its weight is asked for a per-leaf scorer supplier. */
    private static final class CountingQuery extends Query {
        private final Query delegate;
        private final AtomicInteger scorerSupplierCalls;

        CountingQuery(final Query delegate, final AtomicInteger scorerSupplierCalls) {
            this.delegate = delegate;
            this.scorerSupplierCalls = scorerSupplierCalls;
        }

        @Override
        public Weight createWeight(final IndexSearcher searcher, final ScoreMode scoreMode, final float boost) throws IOException {
            final Weight inner = searcher.createWeight(delegate, scoreMode, boost);
            return new Weight(this) {
                ...
                public ScorerSupplier scorerSupplier(final LeafReaderContext context) throws IOException {
                    scorerSupplierCalls.incrementAndGet();
        ...
        public String toString(final String field) {
            return "CountingQuery(" + delegate.toString(field) + ")";
        }
