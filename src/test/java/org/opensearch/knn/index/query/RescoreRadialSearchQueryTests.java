...
public class RescoreRadialSearchQueryTests extends KNNTestCase {
    ...
    public void testCreateWeight_defersSearchUntilScorerSupplier() throws IOException {
        try (Directory directory = newDirectory()) {
            try (IndexWriter writer = new IndexWriter(directory, newIndexWriterConfig())) {
                addVectors(writer, new float[][] { QUERY_TARGET });
                writer.commit();
            }

            try (IndexReader reader = DirectoryReader.open(directory)) {
                final IndexSearcher searcher = new IndexSearcher(reader);
                final Query innerQuery = mock(Query.class);
                final Weight innerWeight = mock(Weight.class);
                when(innerQuery.createWeight(searcher, ScoreMode.TOP_SCORES, 1.0f)).thenReturn(innerWeight);
                ...
                verify(innerQuery, never()).createWeight(searcher, ScoreMode.TOP_SCORES, 1.0f);

                assertNull(weight.scorerSupplier(reader.leaves().get(0)));
                assertNull(weight.scorerSupplier(reader.leaves().get(0)));
                verify(innerQuery).createWeight(searcher, ScoreMode.TOP_SCORES, 1.0f);
            }
        }
    }
    ...
    public void testIsCacheable() {
        try (Directory directory = newDirectory()) {
            ...
            try (IndexReader reader = DirectoryReader.open(directory)) {
                ...
                assertFalse("lazy shard-level results should not be cached", weight.isCacheable(reader.leaves().get(0)));
            }
        }
    }
