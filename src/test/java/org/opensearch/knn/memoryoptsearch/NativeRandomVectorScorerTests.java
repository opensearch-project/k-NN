...
public class NativeRandomVectorScorerTests extends KNNTestCase {
    ...
    @Test
    public void bf16MaxL2Test() {
        // Single MemorySegment scenario
        doBf16ScoringTest(SimdVectorComputeService.SimilarityFunctionType.BF16_L2, KNNVectorSimilarityFunction.EUCLIDEAN, false);

        // Multi MemorySegments scenario
        doBf16ScoringTest(SimdVectorComputeService.SimilarityFunctionType.BF16_L2, KNNVectorSimilarityFunction.EUCLIDEAN, true);
    }

    @SneakyThrows
    private void doBf16ScoringTest(
        final SimdVectorComputeService.SimilarityFunctionType functionType,
        final KNNVectorSimilarityFunction similarityFunction,
        final boolean multiSegmentsScenario
    ) {
        ...
        try (
            ...
            for (int i = 0; i < numVectors; ++i) {
                ...
                for (int j = 0; j < dimension; ++j) {
                    final float floatValue = ThreadLocalRandom.current().nextFloat();
                    // BF16 encoding: take upper 16 bits of float32
                    final int floatBits = Float.floatToIntBits(floatValue);
                    final short bf16Value = (short) ((floatBits >> 16) & 0xFFFF);
                    shortBuffer.put(bf16Value);
                    vector[j] = Float.intBitsToFloat(bf16Value << 16);
                }
                vectors.add(vector);
            }
            channel.write(byteBuffer);
        ...
        for (int j = 0; j < dimension; ++j) {
            ...
            final int floatBits = Float.floatToIntBits(floatValue);
            final short bf16Value = (short) ((floatBits >> 16) & 0xFFFF);
            queryVec[j] = Float.intBitsToFloat(bf16Value << 16);
        }
        ...
        try (mmapDirectory) {
            try (final IndexInput indexInput = mmapDirectory.openInput(tempFile.getFileName().toString(), IOContext.DEFAULT)) {
                ...
                if (multiSegmentsScenario) {
                    ...
                    // 24600 = 2 (=BF16 byte size) * 123 (=dimension) * 100 (=#vectors)
                    assertEquals(24600, addressAndSize[1]);
                }
