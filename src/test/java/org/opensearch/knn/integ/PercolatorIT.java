...
public class PercolatorIT extends KNNRestTestCase {
    ...
    /**
     * A {@code half_float} field reports its data type from the mapping, while a {@code MemoryIndex} leaf
     * would report float. Both resolve to the same scorer, so the percolated document has to score the same
     * as it would on a normal search.
     */
    @SneakyThrows
    public void testKnnInPercolator_whenHalfFloatField_thenScoresLikeFloat() {
        Request create = new Request("PUT", "/" + INDEX);
        create.setJsonEntity(
            "{\"settings\":{\"index\":{\"knn\":true,\"number_of_shards\":1,\"number_of_replicas\":0}},"
                + "\"mappings\":{\"properties\":{\""
        ...
        // Every component is exactly representable in half precision, so there is no rounding to account for.
        Map<String, Object> response = percolate(List.of(new float[] { 1.0f, 2.0f, 3.0f }), null);
        assertEquals(0, shardFailures(response));
        assertEquals(1, totalHits(response));
        // Identical direction, so cosine scores 1.0.
        assertEquals(1.0d, firstScore(response), 0.0001d);
    }
