...
public class HalfFloatIndexIT extends KNNRestTestCase {
    ...
    /**
     * FLAT defaults to x16, i.e. SQ 1-bit over FP16, so scoring goes through
     * {@link org.opensearch.knn.index.codec.KNN1040Codec.KNN1040ScalarQuantizedVectorScorer}. This branch
     * carries the native {@code SQ_COSINE} kernel (opensearch-project#3386), which emits Lucene-native
     * cosine scores {@code (1 + dot) / 2} directly - the same scale upstream asserts. Vectors are
     * L2-normalized on write and the query is normalized too, so {@code dot} is exactly cosine.
     */
    @SneakyThrows
    public void testHalfFloatFlatIndex_cosineSpace_scoresMatchCosine() {
        assertCosineScoresMatch(buildHalfFloatMapping("cosinesimil"), false);
    }

    /** Lucene HNSW defaults to raw FP16, which scores cosine directly, so it keeps the {@code (1 + dot) / 2} scale. */
    @SneakyThrows
    public void testHalfFloatHnswIndex_cosineSpace_scoresMatchCosine() {
        assertCosineScoresMatch(buildHalfFloatHnswMapping("cosinesimil"), false);
    }

    @SneakyThrows
    private void assertCosineScoresMatch(final String mapping, final boolean maxInnerProductScaled) {
        createKnnIndex(INDEX_NAME, mapping);
        ...
        for (KNNResult result : results) {
            final float cosineScore = expectedCosineScore(queryVector, vectors[Integer.parseInt(result.getDocId()) - 1]);
            // (1 + dot) / 2 -> 1 + dot, see the javadoc on the FLAT test above.
            final float expected = maxInnerProductScaled ? 2 * cosineScore : cosineScore;
    ...
    /**
     * Radial search (max_distance / min_score) over a half_float SQ 1-bit index. Upstream carries this
     * coverage in {@code FaissSQRadialSearchIT} / {@code LuceneSQRadialSearchIT}, whose shared harness
     * ({@code AbstractRadialSearchOnQuantizedIndexIT}) does not exist on this branch, so the same two
     * assertions per engine live here instead.
     */
    @SneakyThrows
    public void testRadialSearch_whenHalfFloatFaissSq_withMaxDistance() {
        assertRadialSearchReturnsHits(buildHalfFloatFaissHnswOnDiskMapping(SpaceType.L2.getValue(), "16x"), "max_distance", 10000.0f);
    }

    @SneakyThrows
    public void testRadialSearch_whenHalfFloatFaissSq_withMinScore() {
        assertRadialSearchReturnsHits(buildHalfFloatFaissHnswOnDiskMapping(SpaceType.L2.getValue(), "16x"), "min_score", 0.0001f);
    }

    @SneakyThrows
    public void testRadialSearch_whenHalfFloatLuceneSq_withMaxDistance() {
        assertRadialSearchReturnsHits(buildHalfFloatHnswSq1BitMapping(SpaceType.L2.getValue()), "max_distance", 10000.0f);
    }

    @SneakyThrows
    public void testRadialSearch_whenHalfFloatLuceneSq_withMinScore() {
        assertRadialSearchReturnsHits(buildHalfFloatHnswSq1BitMapping(SpaceType.L2.getValue()), "min_score", 0.0001f);
    }

    private void assertRadialSearchReturnsHits(final String mapping, final String thresholdType, final float thresholdValue)
        throws Exception {
        createKnnIndex(INDEX_NAME, mapping);
        ...
        assertEquals(200, response.getStatusLine().getStatusCode());
        // The thresholds are deliberately permissive: every doc must clear them, so a zero-hit
        // response means radial search never reached the half_float vectors at all.
        assertEquals(3, parseHits(EntityUtils.toString(response.getEntity())));
    }
