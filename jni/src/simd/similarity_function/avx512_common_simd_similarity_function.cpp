...
// Shared AVX512 similarity-function implementations for the plain AVX512 and
// AVX512-SPR builds. Everything here is identical between the two targets:
// the FP16 kernels, the conversion-based BF16 L2 kernel, and the SQ (ADC) kernels.
// The ONLY piece that differs is AVX512BF16MaxIP (native _mm512_dpbf16_ps on SPR
// vs. FP32-conversion + FMA on plain AVX512), so each target defines that struct
// itself and then includes avx512_common_registration.cpp for the shared
// global instances and selectSimilarityFunction().

#ifndef KNN_AVX512_COMMON_SIMD_SIMILARITY_FUNCTION_CPP
#define KNN_AVX512_COMMON_SIMD_SIMILARITY_FUNCTION_CPP
...
struct AVX512NativeFP16IP final : BaseSimilarityFunction<BulkScoreTransformFunc, ScoreTransformFunc> {
    void calculateSimilarityInBulk(SimdVectorSearchContext* srchContext,
        ...
        // Use the dedicated query buffer, NOT srchContext->tmpBuffer: getVectorPointer() reassembles
        ...
        auto& queryBuf = srchContext->queryConvertedBuffer;
...
#endif  // KNN_AVX512_COMMON_SIMD_SIMILARITY_FUNCTION_CPP
