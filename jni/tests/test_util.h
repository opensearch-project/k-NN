...
namespace test_util {
    ...
    // For our unit tests, we want to ensure that each test tests one function in isolation. So, we add a few utils to
    // perform common library operations

    // -------------------------------- FAISS UTILS ----------------------------------

    faiss::Index * FaissCreateIndex(int dim, const std::string& method, faiss::MetricType metric);
    ...
    faiss::IndexIDMap FaissAddData(faiss::Index * index, std::vector<faiss::Index::idx_t> ids,
    ...
    void FaissQueryIndex(faiss::Index * index, float * query, int k, float * distances, faiss::Index::idx_t * ids);

    void FaissTrainIndex(faiss::Index * index, faiss::Index::idx_t n, const float* x);

    // -------------------------------------------------------------------------------

    // ------------------------------- NMSLIB UTILS ----------------------------------

    similarity::Index<float> * NmslibCreateIndex(int* ids, std::vector<std::vector<float>> dataset,
                                                 similarity::Space<float> * space, const std::string& spaceName,
                                                 const std::vector<std::string>& indexParameters);

    void NmslibWriteIndex(similarity::Index<float> * index, const std::string& indexPath);

    similarity::Index<float> * NmslibLoadIndex(const std::string& indexPath, similarity::Space<float> * space,
                                               const std::string& spaceName,
                                               const std::vector<std::string>& queryParameters) ;

    similarity::KNNQuery<float> * NmslibQueryIndex(similarity::Index<float> * index, float * query, int k, int dim,
