...
std::vector<int8_t> test_util::GenerateVectorWithMean(int dim, float targetMean, float variance) {
    std::vector<int8_t> vector(dim);
    std::random_device r;
    std::default_random_engine e1(r());
    std::normal_distribution<float> distribution(targetMean, variance);
    
    float sum = 0.0f;
    for (int i = 0; i < dim; i++) {
        float val = std::max(-128.0f, std::min(127.0f, distribution(e1)));
        vector[i] = static_cast<int8_t>(val);
        sum += vector[i];
    }
    
    // Adjust to hit target mean
    float currentMean = sum / dim;
    float adjustment = targetMean - currentMean;
    for (int i = 0; i < dim; i++) {
        float newVal = vector[i] + adjustment;
        vector[i] = static_cast<int8_t>(std::max(-128.0f, std::min(127.0f, newVal)));
    }
    
    return vector;
}

test_util::DimensionStats test_util::ComputeDimensionStats(
        const std::vector<int8_t>& vectors,
        const std::vector<uint8_t>& codes,
        int dim,
        int numVectors) {
    DimensionStats stats;
    stats.zero_means.resize(dim, 0.0f);
    stats.one_means.resize(dim, 0.0f);
    stats.zero_counts.resize(dim, 0);
    stats.one_counts.resize(dim, 0);

    for (int i = 0; i < numVectors; i++) {
        for (int d = 0; d < dim; d++) {
            bool is_one = codes[i] & (1 << (d % 8));
            float value = vectors[i * dim + d];
            
            if (is_one) {
                stats.one_means[d] += value;
                stats.one_counts[d]++;
            } else {
                stats.zero_means[d] += value;
                stats.zero_counts[d]++;
            }
        }
    }
    ...
    // Compute means
    for (int d = 0; d < dim; d++) {
        if (stats.zero_counts[d] > 0) {
            stats.zero_means[d] /= stats.zero_counts[d];
        }
        if (stats.one_counts[d] > 0) {
            stats.one_means[d] /= stats.one_counts[d];
        }
    }
...
std::vector<float> test_util::TransformQueryADC(
        const std::vector<float>& query,
        const DimensionStats& stats) {
    std::vector<float> transformed(query.size());
    
    for (size_t d = 0; d < query.size(); d++) {
        float denominator = stats.one_means[d] - stats.zero_means[d];
        if (std::abs(denominator) < 1e-6) {
            transformed[d] = 0.0f;
        } else {
            transformed[d] = (query[d] - stats.zero_means[d]) / denominator;
        }
    }
...
std::vector<uint8_t> test_util::QuantizeVectors(
    const std::vector<int8_t>& vectors,
    int dim,
    int numVectors,
    int bits_per_component) {
std::vector<uint8_t> codes(numVectors * ((dim * bits_per_component + 7) / 8));

for (int i = 0; i < numVectors; i++) {
    for (int d = 0; d < dim; d++) {
        int value = vectors[i * dim + d];
        if (bits_per_component == 1) {
            // Simple sign-based quantization for 1-bit
            if (value > 0) {
                codes[i] |= (1 << d);
            }
        }
        // TODO: Add support for 2-bit and 4-bit quantization
    }
}
